...
public class TimetableController {
    ...
    @PutMapping("/{courseId}")
    public ResponseEntity<TimetableDetailResponse> updateTimetable(
            @AuthenticationPrincipal CustomUserPrincipal principal,
            @PathVariable Long courseId,
            @Valid @RequestBody TimetableSaveRequest request
    ) {
        TimetableDetailResponse response =
                timetableService.updateTimetable(principal.userId(), courseId, request);
