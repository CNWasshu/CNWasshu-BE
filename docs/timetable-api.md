...
타임테이블 API는 사용자가 여행 기간과 날짜별 일정을 구성하고 저장·조회·수정·삭제하는 기능을 담당한다.
...
- 요청 사용자가 소유한 타임테이블만 조회·수정·삭제할 수 있다.
- `CourseType.USER`가 아닌 코스는 타임테이블 API로 수정·삭제하지 않는다.
...
- `Course.deletedAt`을 기록하는 소프트 삭제를 수행한다.
